...
import { fetchlessons } from "../../storecategories"
...
export async function readLessons() {
    ...
    const jsonData = process.env.READ_DATA == 'online' ? await fetchlessons() : JSON.parse(fs.readFileSync(path.join(process.cwd(), 'public/data/lessons.json'), 'utf-8'))
