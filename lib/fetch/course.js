...
export async function readCourses() {
    const jsonData = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'public/data/courses.json'), 'utf-8'))
...
export async function getCoursesBasics(cat_slug) {
    ...
    .then(courses => courses.filter(({attributes}) => attributes.Slug != 'news'))
    .then(allcourses => allcourses.filter(attributes => attributes).filter(({attributes}) => attributes.category.data.attributes.Slug == cat_slug))
    .then(async courses => ({
...
export const course_count = async (cat_slug) => {
    ...
    .then(courses => courses.filter(({attributes}) => attributes.Slug != 'blog'))
    .then(courses => courses.filter(({attributes}) => attributes.Slug != 'news'))
    .then(allcourses => allcourses.map(({attributes}) => attributes.category.data).filter((attributes) => attributes).filter(({attributes}) => attributes.Slug == cat_slug).length)
}